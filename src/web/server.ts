...
async function fixFile(filePath) {
  ...
  // Update button visibility in the action row
  const allBtns = [...resultFile.querySelectorAll('.action-row button')];
  const codeBtn = allBtns.find(b => b.textContent === 'Show SVG code' || b.textContent === 'Hide SVG code');
  if (codeBtn) codeBtn.textContent = 'Hide SVG code';
  const dlBtn = allBtns.find(b => b.textContent === 'Download fixed SVG');
  const toggleBtn = allBtns.find(b => b.getAttribute('data-toggle-id'));
  if (dlBtn) dlBtn.style.display = '';
  if (toggleBtn) { toggleBtn.style.display = ''; toggleBtn.textContent = 'Show original'; }
  // Show applied fixes below code
  ...
  fixContainer.innerHTML = appliedHtml;
  fixContainer.style.display = 'block';
}
...
function renderResults(results) {
  ...
  for (const r of issues) {
    ...
    html += '<div class="action-row" style="display:flex;gap:0.5rem;margin-top:0.75rem;margin-bottom:0.75rem;align-items:center;">';
    ...
    html += '<button class="btn btn-secondary" style="margin-top:0;display:none;" onclick="downloadFixed(\\'' + esc(r.filePath) + '\\')">Download fixed SVG</button>';
